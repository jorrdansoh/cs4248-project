...
def features(X_train):
    ...
    empath_features = X_train.apply(lambda x: lexicon.analyze(x, normalize=True))
    empath_features = empath_features.apply(pd.Series)
    empath_features = np.array(empath_features) 
    stop_words_features = np.array(stop_words_features).reshape(-1, 1)



    result = np.hstack((stop_words_features, empath_features))
    # max abs scaler
...
model = ImbPipeline(
    steps=[
        ...
        ("classifier", MLPClassifier(random_state=26, hidden_layer_sizes=(128, 64, 32) , max_iter=250, verbose=True)),
    ],
    verbose=True,
)
param_grid = {
